...
- [dbt/target/compiled/ecommerce_demo/tests/schema.yml/not_null_silver_events_event_id.sql](../dbt/target/compiled/ecommerce_demo/tests/schema.yml/not_null_silver_events_event_id.sql) compiles to a correct query against ``dwh`.`silver_events``
- [dbt/target/compiled/ecommerce_demo/tests/schema.yml/unique_silver_events_event_id.sql](../dbt/target/compiled/ecommerce_demo/tests/schema.yml/unique_silver_events_event_id.sql) compiles to a correct query against ``dwh`.`silver_events``
...
- Changed the `jobs` service to install dependencies from [jobs/requirements.txt](../jobs/requirements.txt)

Added [jobs/requirements.txt](../jobs/requirements.txt):
...
Updated [docker-compose.yml](../docker-compose.yml) to resolve runtime failures:
...
Updated [dbt/dbt_project.yml](../dbt/dbt_project.yml):

- Added `+replication_num: 1` for single-backend local Doris

Added [dbt/macros/generate_schema_name.sql](../dbt/macros/generate_schema_name.sql):

- Prevents dbt from generating schema names like `dwh_dwh` and `dwh_analytics`

Updated [dbt/tests/schema.yml](../dbt/tests/schema.yml):
